...
import { formatDate, formatInteger, formatKzt, formatKztCompact } from '../model/format';
...
export function StatsBar({ stats }: { stats: AnalysisStats }) {
	const items = [
		...
		{ label: 'Период', title: undefined, value: `${formatDate(stats.periodFrom)} – ${formatDate(stats.periodTo)}` },
	];

	return (
		<dl className="border-border bg-surface grid grid-cols-2 gap-x-6 gap-y-3 rounded-lg border px-5 py-3 sm:grid-cols-3 lg:grid-cols-5">
			{items.map((item) => (
				<div className="min-w-0" key={item.label}>
					<dt className="text-fg-muted text-xs">{item.label}</dt>
					<dd className="text-fg tabular truncate text-lg font-semibold" title={item.title}>
						{item.value}
					</dd>
				</div>
			))}
